...
import { NearRpcClient, viewAccessKey, broadcastTxCommit, viewFunction, parseCallResultToJson, block, gasPrice } from '@near-js/jsonrpc-client';
...
if (!accountId || !privateKey) {
  console.error('❌ Error: Please set NEAR_ACCOUNT_ID and NEAR_PRIVATE_KEY environment variables');
...
try {
  ...
  if (result.status && typeof result.status === 'object' && 'SuccessValue' in result.status) {
    ...
    if (result.status.SuccessValue) {
      ...
      const decodedResult = Buffer.from(result.status.SuccessValue, 'base64').toString();
      console.log(`   📤 Return value: ${decodedResult || '(empty)'}`);
    }
  } else if (result.status && typeof result.status === 'object' && 'Failure' in result.status) {
    console.log('   ❌ Transaction failed:', result.status.Failure);
  }
  
  // Show gas usage
  // Get gas price from a separate call
  const gasPriceResult = await gasPrice(client, { blockId: result.transactionOutcome?.blockHash });
