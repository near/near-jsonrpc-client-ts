...
export { NearRpcClient } from '../../client';
export type * from '../../types';
// Export convenience functions (including JSON parsing utilities)
...
`;
  ...
  // Generate no-validation convenience functions
  const noValidationConveniencePath = join(noValidationDir, 'convenience.ts');
  const noValidationConvenienceContent = `// No-validation convenience functions for version ${versionDir}
...
`;
  writeFileSync(noValidationConveniencePath, noValidationConvenienceContent);
}
