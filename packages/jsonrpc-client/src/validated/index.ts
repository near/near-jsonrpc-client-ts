...
// Generated at: 2025-12-24T06:06:08.514Z
// Total functions: 39
//
...
import {
  ...
  RpcViewGasKeyListRequestSchema,
  RpcViewGasKeyListResponseSchema,
  RpcViewGasKeyRequestSchema,
  RpcViewGasKeyResponseSchema,
...
export async function experimentalViewGasKey(
  client: NearRpcClient,
  params: RpcViewGasKeyRequest
): Promise<RpcViewGasKeyResponse> {
  // Validate request parameters
  const requestSchema = RpcViewGasKeyRequestSchema();
  if (params !== undefined) {
    try {
      requestSchema.parse(params);
    } catch (error) {
  ...
  // Call the base function
  const result = await baseFunctions.experimentalViewGasKey(client, params);

  // Validate response
  const responseSchema = RpcViewGasKeyResponseSchema();
  try {
    // Wrap in JSON-RPC response format for validation
    responseSchema.parse(result);
  } catch (error) {
...
export async function experimentalViewGasKeyList(
  client: NearRpcClient,
  params: RpcViewGasKeyListRequest
): Promise<RpcViewGasKeyListResponse> {
  // Validate request parameters
  const requestSchema = RpcViewGasKeyListRequestSchema();
  if (params !== undefined) {
    try {
      requestSchema.parse(params);
    } catch (error) {
  ...
  // Call the base function
  const result = await baseFunctions.experimentalViewGasKeyList(client, params);

  // Validate response
  const responseSchema = RpcViewGasKeyListResponseSchema();
  try {
    // Wrap in JSON-RPC response format for validation
    responseSchema.parse(result);
  } catch (error) {
